...
`-drive IMG[,ro]` attach virtio-blk disks, `-share HOSTDIR[,tag=TAG][,ro]` expose a host
directory through virtio-9p (repeatable; `ro` rejects guest-side mutations; default tags are `hostshare`,
